...
const RegStudentDetails = () => {
  ...
  const getStudentData = async () => {
    const response = await axios.get(
      `http://localhost:4000/api/auth/getAllRegStudent`
    );
    const data = response.data;
    console.log(data);
    if (classFilter) {
      const filteredData = data.filter((item) => item.class === classFilter);
  ...
  const downloadgetOnlyRegStudent = async () => {
    ...
    window.location.href =
      "http://localhost:4000/api/auth/downloadgetOnlyRegStudent";
  };
